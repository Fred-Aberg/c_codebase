			...
			for (uint i = 0; i < n_subcontainers; i++)
			{
				...
				if (c_subcontainer.size_type == TILES)
				{
					if (c_data.orientation == HORIZONTAL)
					{
						...
						if (sub_y1 > y1 - 1)
							break;
						tiles_drawn += ascui_draw_container(grid, c_subcontainer, sub_x0, sub_y0 + tiles_drawn, sub_x1, sub_y1, c_data.orientation);
					}
					else if (c_data.orientation == VERTICAL)
					{
						...
						if (sub_x1 > x1 - 1)
							break;
						tiles_drawn += ascui_draw_container(grid, c_subcontainer, sub_x0 + tiles_drawn, sub_y0, sub_x1, sub_y1, c_data.orientation);
					} 					
				}
				else if (c_subcontainer.size_type == PERCENTAGE)
				{
